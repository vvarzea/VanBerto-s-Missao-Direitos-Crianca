...
window.addEventListener("DOMContentLoaded", () => {
  ...
  // ===== TEMAS visuais — 20 temas únicos, um por nível =====
  const THEMES = [
    { skyTop:0x1a6ab5, skyBot:0x8ed6f8, hillColor:0x2e9e52, grassTop:0x44cc6a }, //  0 · N1  céu azul clássico
    { skyTop:0x3d1466, skyBot:0xff8c40, hillColor:0xbf3c0f, grassTop:0xd95210 }, //  1 · N2  crepúsculo roxo-laranja
    { skyTop:0x006680, skyBot:0x50e8e0, hillColor:0x0a7a6a, grassTop:0x18c0b0 }, //  2 · N3  aqua profundo
    { skyTop:0xc02880, skyBot:0xffb8d8, hillColor:0xd0408a, grassTop:0xf060aa }, //  3 · N4  rosa vibrante
    { skyTop:0x1a0060, skyBot:0xb060ff, hillColor:0x5010a0, grassTop:0x7830d8 }, //  4 · N5  lilás noturno
    { skyTop:0x004060, skyBot:0x10d4cc, hillColor:0x006868, grassTop:0x00b0a8 }, //  5 · N6  turquesa
    { skyTop:0x7a2000, skyBot:0xffb060, hillColor:0xd05010, grassTop:0xf07030 }, //  6 · N7  laranja quente
    { skyTop:0x001a5a, skyBot:0x2090e8, hillColor:0x0050a0, grassTop:0x1878d0 }, //  7 · N8  azul noturno vivo
    { skyTop:0x5a0030, skyBot:0xff80b8, hillColor:0xb02070, grassTop:0xd83090 }, //  8 · N9  magenta rico
    { skyTop:0x0a2010, skyBot:0x40b858, hillColor:0x1a5e2a, grassTop:0x28904a }, //  9 · N10 floresta verde
    { skyTop:0x4a0800, skyBot:0xff6040, hillColor:0xcc2800, grassTop:0xff4418 }, // 10 · N11 vermelho vulcão
    { skyTop:0x001828, skyBot:0x0090c0, hillColor:0x005070, grassTop:0x00a8d0 }, // 11 · N12 azul oceano profundo
    { skyTop:0x281800, skyBot:0xd4a040, hillColor:0x806010, grassTop:0xa07820 }, // 12 · N13 dourado árido (deserto)
    { skyTop:0x080820, skyBot:0x4040c0, hillColor:0x202080, grassTop:0x3030a0 }, // 13 · N14 azul meia-noite
    { skyTop:0x006020, skyBot:0x80ff80, hillColor:0x008040, grassTop:0x20cc60 }, // 14 · N15 verde brilhante primavera
    { skyTop:0x300040, skyBot:0xc080ff, hillColor:0x600080, grassTop:0x8020c0 }, // 15 · N16 violeta escuro
    { skyTop:0x602000, skyBot:0xffa020, hillColor:0xc06000, grassTop:0xe08010 }, // 16 · N17 âmbar outonal
    { skyTop:0x002040, skyBot:0x00d0a0, hillColor:0x004840, grassTop:0x00907a }, // 17 · N18 verde-azulado noturno
    { skyTop:0x200830, skyBot:0xff40a0, hillColor:0x801060, grassTop:0xc01880 }, // 18 · N19 púrpura aurora
    { skyTop:0xff6a1a, skyBot:0xffe39a, hillColor:0xe0871a, grassTop:0x5ec85a }, // 19 · N20 FINAL festivo pôr-do-sol
  ];
  ...
  const LEVELS = [
    {
      ...
      malwares:[
        {x:1040,y:422,vx:0,pattern:"mini"},      // em cima da plataforma y:450
        {x:1960,y:352,vx:-150,pattern:"patrol"}  // em cima da plataforma y:380 (a maior)
      ]
    },
    {
      name: "Nível 2 — A Declaração de 1959",
      theme:1, quizTheme:"declaracao", worldW:2800,
      spawn:{x:120,y:464}, doorX:2540,
      // ══ ZIG-ZAG COM VÃO ARMADILHA + FINAL ESTREITO ══
      // Início confortável → vão grande inesperado (~320px) → estreitas rápidas
      platforms:[
        {x:120,y:520,w:500,h:28},

        // Fase 1 — vãos pequenos/médios
        {x:760,y:420,w:220,h:22},
        {x:1000,y:350,w:200,h:22},
        {x:1180,y:420,w:180,h:22},
        {x:1400,y:340,w:210,h:22},

        // Fase 2 — depois do vão grande
        {x:1900,y:280,w:190,h:22},

        // Fase 3 — estreitas em zigue-zague
        {x:2120,y:350,w:110,h:22},
        {x:2270,y:420,w:110,h:22},
        {x:2410,y:350,w:110,h:22},
        {x:2560,y:520,w:600,h:28},
      ],
      items:[
        {x:760,y:370,kind:"balao"},
        {x:1000,y:300,kind:"brinquedo"},
        {x:1400,y:290,kind:"estrela"},
        {x:1900,y:230,kind:"duplosalto"},  // recompensa depois do vão difícil
        {x:2270,y:370,kind:"medalha"},
      ],
      malwares:[
        // mini no início — patrulha a 1ª ilha
        {x:760,y:396,vx:0,pattern:"mini"},
        // patrol na fase 1 — percorre a 4ª plataforma
        {x:1400,y:316,vx:120,pattern:"patrol"},
        // patrol depois do vão — guarda a recompensa
        {x:1900,y:252,vx:0,pattern:"patrol"},
        // mini nas estreitas — na plataforma do meio
        {x:2270,y:396,vx:0,pattern:"mini"},
      ],
      trampolines:[{x:1650,y:510}],
      secrets:[{x:1180,y:368,kind:"estrela",points:20}],
    },
    {
      name: "Nível 3 — A Convenção de 1989",
      theme:2, quizTheme:"convencao", worldW:2900,
      spawn:{x:120,y:464}, doorX:2650,
      // ══ TORRE CENTRAL — sobe e desce ══
      // Em vez de ir sempre para a direita, há uma estrutura em pirâmide
      // que força o jogador a subir 5 andares e depois descer do outro lado.
      // O portal está na base direita — ao mesmo nível do spawn.
      // Novidade: primeira aparição de um jumper (no topo da torre).
      platforms:[
        // Base esquerda — arranque
        {x:120,y:520,w:480,h:28},

        // Subida da torre — lado esquerdo
        {x:700,y:450,w:200,h:22},      // andar 1
        {x:900,y:370,w:180,h:22},      // andar 2
        {x:1100,y:295,w:170,h:22},     // andar 3
        {x:1300,y:220,w:160,h:22},     // andar 4

        // Topo da torre
        {x:1500,y:155,w:200,h:22},     // topo — plataforma mais alta do nível

        // Descida — lado direito
        {x:1700,y:220,w:160,h:22},     // andar 4 direita
        {x:1900,y:295,w:170,h:22},     // andar 3 direita
        {x:2100,y:370,w:180,h:22},     // andar 2 direita
        {x:2300,y:450,w:200,h:22},     // andar 1 direita

        // Base direita — chegada
        {x:2700,y:520,w:600,h:28},
      ],
      items:[
        {x:700,y:400,kind:"balao"},
        {x:1100,y:245,kind:"estrela"},
        {x:1500,y:105,kind:"medalha"},  // item no topo — recompensa máxima
        {x:1900,y:245,kind:"duplosalto"},
        {x:2300,y:400,kind:"brinquedo"},
        {x:120,y:470,kind:"heart"},
      ],
      malwares:[
        // mini no andar 1 esquerdo
        {x:700,y:426,vx:0,pattern:"mini"},
        // patrol no andar 2 esquerdo
        {x:900,y:346,vx:100,pattern:"patrol"},
        // jumper no TOPO — primeiro encontro com jumper, numa plataforma larga
        {x:1500,y:127,vx:0,pattern:"jumper"},
        // patrol no andar 2 direito
        {x:1900,y:271,vx:-100,pattern:"patrol"},
        // mini no andar 1 direito
        {x:2300,y:426,vx:0,pattern:"mini"},
      ],
      secrets:[{x:1300,y:168,kind:"estrela",points:25}],
    },
    {
      name: "Nível 4 — O Direito ao Brincar",
      theme:3, quizTheme:"brincar", worldW:2800,
      spawn:{x:80,y:464}, doorX:2630,
      // ══ TRAMPOLINS OBRIGATÓRIOS ══ (mantido — está bem)
      platforms:[
      ...
      malwares:[
        // Todos em cima das plataformas das ilhas
        {x:500,y:362,vx:0,pattern:"mini"},
        {x:880,y:402,vx:0,pattern:"mini"},
        {x:1260,y:342,vx:100,pattern:"patrol"},
        {x:1640,y:392,vx:-100,pattern:"patrol"},
        {x:2020,y:332,vx:0,pattern:"jumper"},
      ],
      secrets:[{x:1060,y:415,kind:"estrela",points:20}],
    },
    {
      name: "Nível 5 — O Direito à Educação",
      theme:4, quizTheme:"educacao", worldW:3200,
      spawn:{x:120,y:464}, doorX:3000,
      // ══ DOIS CORREDORES + CRUZAMENTOS ══
      // O nível tem um corredor alto (y≈240) e um corredor baixo (y≈420).
      // Há 3 "cruzamentos" — plataformas que ligam os dois pisos —
      // onde estão os itens mais valiosos. O jogador tem de decidir
      // quando vale a pena mudar de corredor.
      // Primeira aparição de plataformas móveis (lentas, previsíveis).
      platforms:[
        // Arranque
        {x:120,y:520,w:480,h:28},

        // ── Corredor BAIXO (y≈420) ────────────────────────────────
        {x:700,y:420,w:200,h:22},
        {x:1050,y:430,w:180,h:22},
        {x:1600,y:410,w:200,h:22},
        {x:1950,y:420,w:180,h:22},
        {x:2500,y:415,w:200,h:22},

        // ── Corredor ALTO (y≈240) ─────────────────────────────────
        {x:850,y:240,w:180,h:22},
        {x:1200,y:255,w:160,h:22},
        {x:1750,y:240,w:180,h:22},
        {x:2100,y:250,w:160,h:22},
        {x:2650,y:240,w:180,h:22},

        // ── Cruzamentos (ligam os dois pisos) ─────────────────────
        {x:1400,y:330,w:140,h:22},   // cruzamento 1 — meio do nível
        {x:2300,y:335,w:140,h:22},   // cruzamento 2

        // Chegada
        {x:3050,y:520,w:600,h:28},
      ],
      items:[
        // Corredor baixo — itens simples
        {x:700,y:370,kind:"balao"},
        {x:1600,y:360,kind:"brinquedo"},
        // Cruzamentos — itens médios
        {x:1400,y:280,kind:"estrela"},
        {x:2300,y:285,kind:"medalha"},
        // Corredor alto — melhor item
        {x:2650,y:190,kind:"duplosalto"},
        {x:120,y:470,kind:"heart"},
      ],
      malwares:[
        // Patrulhas no corredor baixo
        {x:700,y:392,vx:100,pattern:"patrol"},
        {x:1950,y:392,vx:-110,pattern:"patrol"},
        // Mini nos cruzamentos — obrigam a saltar por cima
        {x:1400,y:302,vx:0,pattern:"mini"},
        {x:2300,y:307,vx:0,pattern:"mini"},
        // Jumper no corredor alto — mais perigoso lá em cima
        {x:2100,y:222,vx:0,pattern:"jumper"},
      ],
      movingPlatforms:[
        // Ponte móvel horizontal entre corredor baixo e alto — lenta
        {x:1200,y:335,w:130,h:22,rangeX:0,rangeY:90,speed:45},
        // Ponte móvel perto da chegada
        {x:2850,y:335,w:130,h:22,rangeX:0,rangeY:90,speed:50},
      ],
      secrets:[{x:850,y:188,kind:"medalha",points:20}],
    },
    {
      name: "Nível 6 — O Direito à Saúde",
      theme:5, quizTheme:"saude", worldW:3100,
      spawn:{x:120,y:464}, doorX:2850,
      // ══ CLUSTERS — ilhas internas + vãos grandes ══
      // Em vez de plataformas isoladas, grupos de 3 plataformas muito juntas
      // (fácil saltar dentro do cluster) separados por vãos grandes
      // (exige timing e atenção para atravessar).
      // Metáfora: dentro de cada "grupo" estás protegido; entre grupos tens de te expor.
      platforms:[
        // Arranque
        {x:120,y:520,w:400,h:28},

        // ── Cluster A ─────────────────────────────────────────────
        {x:680,y:420,w:160,h:22},
        {x:860,y:340,w:150,h:22},   // sobe dentro do cluster
        {x:1020,y:420,w:150,h:22},  // desce — forma um "U"

        // ── VAO GRANDE A (~280px) ─────────────────────────────────

        // ── Cluster B ─────────────────────────────────────────────
        {x:1380,y:380,w:160,h:22},
        {x:1560,y:280,w:150,h:22},  // pico do cluster
        {x:1720,y:380,w:150,h:22},

        // ── VAO GRANDE B (~260px) ─────────────────────────────────

        // ── Cluster C ─────────────────────────────────────────────
        {x:2060,y:440,w:160,h:22},
        {x:2230,y:340,w:150,h:22},
        {x:2390,y:440,w:150,h:22},

        // ── Chegada ───────────────────────────────────────────────
        {x:2860,y:520,w:600,h:28},
      ],
      items:[
        // Um item no pico de cada cluster — recompensa escalar
        {x:860,y:290,kind:"estrela"},
        {x:1560,y:230,kind:"medalha"},
        {x:2230,y:290,kind:"duplosalto"},
        // Item no arranque — fácil
        {x:680,y:370,kind:"balao"},
        // Coração na chegada
        {x:120,y:470,kind:"heart"},
      ],
      malwares:[
        // Mini no fundo de cada cluster — guarda a entrada
        {x:680,y:392,vx:0,pattern:"mini"},
        {x:1380,y:352,vx:0,pattern:"mini"},
        {x:2060,y:412,vx:0,pattern:"mini"},
        // Patrol no pico do cluster B — mais difícil de evitar
        {x:1560,y:252,vx:110,pattern:"patrol"},
        // Jumper no pico do cluster C
        {x:2230,y:312,vx:0,pattern:"jumper"},
      ],
      trampolines:[
        {x:1250,y:510},   // ajuda a atravessar o vão A
        {x:1940,y:510},   // ajuda a atravessar o vão B
      ],
      secrets:[{x:1020,y:368,kind:"brinquedo",points:20}],
    },
    {
    ...
    },
    {
      ...
      spawn:{x:120,y:464}, doorX:3050,
      // ══ PARLAMENTO — plataformas em semicírculo + vão obrigatório ══
      // As plataformas formam dois "braços" que sobem da base para o centro,
      // deixando um grande vão no meio. O trampolim é a única ponte.
      // Metáfora: para participar tens de chegar ao centro — e não é fácil.
      platforms:[
        {x:120,y:520,w:400,h:28},

        // Braço esquerdo — sobe
        {x:700,y:440,w:180,h:22},
        {x:900,y:360,w:160,h:22},
        {x:1080,y:280,w:150,h:22},
        {x:1240,y:210,w:140,h:22},          // pico esquerdo

        // VAO CENTRAL (~400px) — trampolim obrigatório no chão

        // Braço direito — desce
        {x:1820,y:210,w:140,h:22},          // pico direito (simétrico)
        {x:1980,y:280,w:150,h:22},
        {x:2160,y:360,w:160,h:22},
        {x:2360,y:440,w:180,h:22},

        // Sequência final irregular
        {x:2620,y:360,w:130,h:22},
        {x:2820,y:280,w:120,h:22},
        {x:3100,y:520,w:500,h:28},
      ],
      items:[
        {x:700,y:390,kind:"balao"},
        {x:1080,y:230,kind:"estrela"},
        {x:1240,y:160,kind:"medalha"},       // no pico esquerdo
        {x:1820,y:160,kind:"duplosalto"},    // no pico direito — recompensa quem atravessou
        {x:2160,y:310,kind:"brinquedo"},
        {x:2820,y:230,kind:"heart"},
      ],
      malwares:[
        {x:700,y:412,vx:0,pattern:"mini"},
        {x:900,y:332,vx:100,pattern:"patrol"},
        {x:1240,y:182,vx:0,pattern:"jumper"},   // guarda o pico esquerdo
        {x:1820,y:182,vx:0,pattern:"jumper"},   // guarda o pico direito
        {x:2360,y:412,vx:-110,pattern:"patrol"},
        {x:2620,y:332,vx:0,pattern:"mini"},
      ],
      trampolines:[{x:1530,y:510}],            // único caminho para atravessar o vão
      secrets:[{x:1530,y:300,kind:"estrela",points:30}],
    },
    {
      name: "Nível 9 — O Futuro Sustentável",
      theme:8, quizTheme:"futuro", worldW:3400,
      spawn:{x:120,y:464}, doorX:3150,
      // ══ ESPIRAL DESCENDENTE — começa alto, desce em caracol ══
      // Completamente diferente dos outros: o jogador começa no topo
      // (subida rápida no início) e depois desce em espiral larga.
      // A porta está lá em baixo. Os melhores itens estão no topo.
      platforms:[
        {x:120,y:520,w:380,h:28},

        // Subida rápida para o topo
        {x:560,y:420,w:150,h:22},
        {x:760,y:320,w:140,h:22},
        {x:940,y:220,w:140,h:22},
        {x:1120,y:140,w:160,h:22},          // topo do nível

        // Descida em espiral — lado direito, vai descendo gradualmente
        {x:1380,y:190,w:150,h:22},
        {x:1620,y:260,w:160,h:22},
        {x:1860,y:340,w:150,h:22},
        {x:2100,y:420,w:160,h:22},

        // Curva no fundo — volta para a esquerda
        {x:2340,y:460,w:140,h:22},
        {x:2560,y:380,w:140,h:22},
        {x:2760,y:300,w:140,h:22},
        {x:2960,y:380,w:130,h:22},
        {x:3200,y:520,w:500,h:28},
      ],
      items:[
        {x:560,y:370,kind:"balao"},
        {x:940,y:170,kind:"estrela"},
        {x:1120,y:90,kind:"medalha"},        // topo — melhor item
        {x:1620,y:210,kind:"duplosalto"},
        {x:2340,y:410,kind:"brinquedo"},
        {x:2760,y:250,kind:"heart"},
      ],
      malwares:[
        {x:760,y:292,vx:0,pattern:"mini"},
        {x:1120,y:112,vx:0,pattern:"jumper"},   // guarda o topo
        {x:1620,y:232,vx:110,pattern:"patrol"},
        {x:2100,y:392,vx:-100,pattern:"patrol"},
        {x:2560,y:352,vx:0,pattern:"jumper"},
      ],
      movingPlatforms:[
        {x:1380,y:165,w:130,h:22,rangeX:120,rangeY:0,speed:70},  // plataforma móvel no início da descida
      ],
      secrets:[{x:940,y:168,kind:"estrela",points:30}],
    },
    {
      name: "Nível 10 — A UNICEF e os Direitos",
      theme:9, quizTheme:"unicef", worldW:3500,
      spawn:{x:120,y:464}, doorX:3250,
      // ══ MAPA MUNDO — 5 "continentes" separados por oceanos ══
      // 5 grupos de plataformas (continentes) separados por vãos largos.
      // Cada continente tem estrutura própria: um tem escada, outro tem plataforma alta isolada, etc.
      // Todos os jumpers — o nível mais difícil até aqui.
      platforms:[
        {x:120,y:520,w:380,h:28},

        // Continente 1 — Europa (escada simples)
        {x:680,y:420,w:170,h:22},
        {x:860,y:330,w:150,h:22},

        // Oceano (~220px)

        // Continente 2 — América (plataforma alta + baixa)
        {x:1180,y:240,w:140,h:22},          // ilha alta
        {x:1380,y:400,w:140,h:22},          // ilha baixa

        // Oceano (~200px)

        // Continente 3 — África (três degraus rápidos)
        {x:1680,y:360,w:120,h:22},
        {x:1820,y:270,w:120,h:22},
        {x:1960,y:360,w:120,h:22},

        // Oceano (~220px)

        // Continente 4 — Ásia (plataforma longa + trampolim)
        {x:2300,y:310,w:200,h:22},

        // Oceano (~200px)

        // Continente 5 — Oceânia (duas ilhas com vão interno)
        {x:2680,y:380,w:130,h:22},
        {x:2900,y:280,w:130,h:22},

        {x:3300,y:520,w:600,h:28},
      ],
      items:[
        {x:680,y:370,kind:"balao"},
        {x:1180,y:190,kind:"estrela"},       // ilha alta — continente 2
        {x:1820,y:220,kind:"medalha"},
        {x:2300,y:260,kind:"duplosalto"},
        {x:2900,y:230,kind:"brinquedo"},
        {x:120,y:470,kind:"heart"},
      ],
      malwares:[
        {x:680,y:392,vx:100,pattern:"jumper"},
        {x:1180,y:212,vx:0,pattern:"jumper"},
        {x:1820,y:242,vx:0,pattern:"jumper"},
        {x:2300,y:282,vx:120,pattern:"jumper"},
        {x:2680,y:352,vx:0,pattern:"jumper"},
      ],
      movingPlatforms:[
        {x:1530,y:320,w:120,h:22,rangeX:0,rangeY:80,speed:60},   // ponte entre continentes 2 e 3
        {x:2520,y:340,w:120,h:22,rangeX:100,rangeY:0,speed:80},  // ponte entre 4 e 5
      ],
      trampolines:[{x:1060,y:510},{x:2130,y:510}],
      secrets:[{x:1380,y:348,kind:"heart"}],
    },
    {
    ...
    },
    {
      ...
      spawn:{x:120,y:464}, doorX:3420,
      // ══ TRÊS CASAS — grupos aconchegantes separados por vãos ══
      // Três "casas" (clusters de 3 plataformas em triângulo)
      // separadas por vãos que exigem trampolim.
      // Dentro de cada casa és seguro; entre casas estás exposto.
      platforms:[
        {x:120,y:520,w:380,h:28},

        // Casa A
        {x:700,y:430,w:180,h:22},           // base
        {x:860,y:320,w:160,h:22},           // meio-direita
        {x:560,y:320,w:160,h:22},           // meio-esquerda

        // Vão (~280px) — trampolim

        // Casa B
        {x:1440,y:400,w:180,h:22},
        {x:1600,y:290,w:160,h:22},
        {x:1280,y:290,w:160,h:22},

        // Vão (~280px) — trampolim

        // Casa C
        {x:2180,y:420,w:180,h:22},
        {x:2340,y:310,w:160,h:22},
        {x:2020,y:310,w:160,h:22},

        // Última secção antes da porta
        {x:2680,y:360,w:150,h:22},
        {x:2880,y:280,w:140,h:22},
        {x:3080,y:360,w:140,h:22},
        {x:3470,y:520,w:600,h:28},
      ],
      items:[
        {x:560,y:270,kind:"estrela"},        // topo esq casa A
        {x:860,y:270,kind:"balao"},          // topo dir casa A
        {x:1280,y:240,kind:"medalha"},       // topo casa B
        {x:1440,y:350,kind:"duplosalto"},
        {x:2020,y:260,kind:"brinquedo"},     // topo casa C
        {x:2880,y:230,kind:"heart"},
      ],
      malwares:[
        {x:700,y:402,vx:0,pattern:"mini"},       // entrada casa A
        {x:860,y:292,vx:0,pattern:"patrol"},     // topo casa A
        {x:1440,y:372,vx:0,pattern:"mini"},      // entrada casa B
        {x:1600,y:262,vx:0,pattern:"jumper"},    // topo casa B
        {x:2180,y:392,vx:0,pattern:"mini"},      // entrada casa C
        {x:2340,y:282,vx:0,pattern:"jumper"},    // topo casa C
      ],
      trampolines:[{x:1080,y:510},{x:1820,y:510}],
      secrets:[{x:3080,y:308,kind:"estrela",points:25}],
    },
    {
      name: "Nível 13 — Os Direitos dos Refugiados",
      theme:12, quizTheme:"refugiados", worldW:3700,
      spawn:{x:120,y:464}, doorX:3450,
      // ══ TRAVESSIA — caminho difícil, instável, sem rede ══
      // Plataformas estreitas (100-120px) muito irregulares em altura,
      // com uma plataforma móvel obrigatória a meio.
      // Uma zona de "terra de ninguém" com hazard no nível 13+ seria ideal,
      // mas sem hazards: vãos longos e plataformas minúsculas criam a tensão.
      platforms:[
        {x:120,y:520,w:380,h:28},

        // Fase 1 — zig-zag irregular
        {x:680,y:400,w:120,h:22},
        {x:840,y:300,w:110,h:22},
        {x:1000,y:420,w:110,h:22},
        {x:1160,y:310,w:110,h:22},
        {x:1320,y:430,w:110,h:22},

        // Vão largo — plataforma móvel obrigatória
        // (plataforma estática de apoio dos dois lados)
        {x:1560,y:380,w:100,h:22},          // trampolim de impulsão
        {x:1840,y:320,w:100,h:22},          // aterragem

        // Fase 2 — mais rápida e alta
        {x:2000,y:230,w:110,h:22},
        {x:2160,y:340,w:100,h:22},
        {x:2320,y:220,w:110,h:22},
        {x:2480,y:330,w:110,h:22},
        {x:2640,y:220,w:110,h:22},
        {x:2820,y:320,w:120,h:22},
        {x:3020,y:420,w:120,h:22},
        {x:3500,y:520,w:600,h:28},
      ],
      items:[
        {x:680,y:350,kind:"balao"},
        {x:1160,y:260,kind:"estrela"},
        {x:1840,y:270,kind:"duplosalto"},    // recompensa depois da travessia
        {x:2320,y:170,kind:"medalha"},
        {x:2640,y:170,kind:"brinquedo"},
        {x:120,y:470,kind:"heart"},
      ],
      malwares:[
        {x:840,y:272,vx:0,pattern:"mini"},
        {x:1320,y:402,vx:0,pattern:"mini"},
        {x:1840,y:292,vx:0,pattern:"patrol"},
        {x:2320,y:192,vx:0,pattern:"jumper"},
        {x:2820,y:292,vx:110,pattern:"patrol"},
      ],
      movingPlatforms:[
        {x:1700,y:350,w:110,h:22,rangeX:110,rangeY:0,speed:85},  // a "barca" obrigatória
      ],
      secrets:[{x:3020,y:368,kind:"estrela",points:30}],
    },
    {
      name: "Nível 14 — Contra o Trabalho Infantil",
      theme:13, quizTheme:"trabalho", worldW:3750,
      spawn:{x:120,y:464}, doorX:3550,
      // ══ FÁBRICA — correia rolante + obstáculos alternados ══
      // Plataformas estáticas largas com vilões PATROL a patrulhar em pares
      // em sentidos opostos (como máquinas). Plataformas móveis são as "correias".
      // O jogador tem de escolher quando atravessar cada "máquina".
      platforms:[
        {x:120,y:520,w:380,h:28},

        // Secção 1
        {x:720,y:400,w:260,h:22},           // longa — 2 vilões a patrulhar
        {x:1100,y:300,w:200,h:22},

        // Secção 2
        {x:1540,y:380,w:260,h:22},          // longa
        {x:1920,y:280,w:200,h:22},

        // Secção 3
        {x:2360,y:360,w:260,h:22},          // longa
        {x:2740,y:260,w:200,h:22},

        // Secção 4
        {x:3160,y:380,w:220,h:22},
        {x:3600,y:520,w:600,h:28},
      ],
      items:[
        {x:720,y:350,kind:"balao"},
        {x:1100,y:250,kind:"estrela"},
        {x:1540,y:330,kind:"duplosalto"},
        {x:1920,y:230,kind:"medalha"},
        {x:2740,y:210,kind:"brinquedo"},
        {x:120,y:470,kind:"heart"},
      ],
      malwares:[
        // Pares a patrulhar nas plataformas longas — em sentidos opostos
        {x:660,y:372,vx:140,pattern:"patrol"},
        {x:780,y:372,vx:-140,pattern:"patrol"},
        {x:1480,y:352,vx:130,pattern:"patrol"},
        {x:1600,y:352,vx:-130,pattern:"patrol"},
        {x:2300,y:332,vx:130,pattern:"patrol"},
        {x:2420,y:332,vx:-130,pattern:"patrol"},
        // Jumpers nos pisos altos
        {x:1100,y:272,vx:0,pattern:"jumper"},
        {x:2740,y:232,vx:0,pattern:"jumper"},
      ],
      movingPlatforms:[
        {x:1300,y:340,w:130,h:22,rangeX:140,rangeY:0,speed:90},   // correia 1
        {x:2120,y:320,w:130,h:22,rangeX:0,rangeY:80,speed:70},    // elevador
        {x:2960,y:320,w:130,h:22,rangeX:130,rangeY:0,speed:100},  // correia 2
      ],
      secrets:[{x:3160,y:328,kind:"estrela",points:30}],
    },
    {
      name: "Nível 15 — O Direito à Expressão",
      theme:14, quizTheme:"expressao", worldW:3800,
      spawn:{x:120,y:464}, doorX:3600,
      // ══ ONDA — plataformas em sinusoide larga ══
      // As plataformas seguem uma curva sinusoidal suave mas ampla:
      // alto → baixo → alto → baixo, com amplitude crescente.
      // Vilões nos picos e trampolins nos vales mais fundos.
      platforms:[
        {x:120,y:520,w:380,h:28},

        // Onda — cristas e vales alternados, amplitude cresce
        {x:680,y:320,w:160,h:22},           // crista 1
        {x:920,y:430,w:150,h:22},           // vale 1
        {x:1140,y:280,w:150,h:22},          // crista 2 (mais alta)
        {x:1370,y:440,w:140,h:22},          // vale 2
        {x:1590,y:240,w:150,h:22},          // crista 3 (ainda mais alta)
        {x:1820,y:450,w:140,h:22},          // vale 3 (fundo)
        {x:2060,y:210,w:150,h:22},          // crista 4 — máximo
        {x:2300,y:430,w:140,h:22},          // vale 4
        {x:2540,y:260,w:150,h:22},          // crista 5
        {x:2780,y:400,w:150,h:22},          // vale 5
        {x:3020,y:300,w:150,h:22},          // crista final
        {x:3650,y:520,w:600,h:28},
      ],
      items:[
        {x:680,y:270,kind:"balao"},
        {x:1140,y:230,kind:"estrela"},
        {x:1590,y:190,kind:"medalha"},
        {x:2060,y:160,kind:"duplosalto"},   // item mais alto do nível
        {x:2540,y:210,kind:"brinquedo"},
        {x:120,y:470,kind:"heart"},
      ],
      malwares:[
        // Patrol nas cristas — "guardas" nos pontos mais visíveis
        {x:680,y:292,vx:100,pattern:"patrol"},
        {x:1140,y:252,vx:0,pattern:"patrol"},
        {x:1590,y:212,vx:0,pattern:"jumper"},   // jumper na crista 3
        {x:2060,y:182,vx:0,pattern:"jumper"},   // jumper no máximo
        {x:2540,y:232,vx:100,pattern:"patrol"},
        {x:3020,y:272,vx:0,pattern:"jumper"},
      ],
      movingPlatforms:[
        {x:1000,y:370,w:120,h:22,rangeX:0,rangeY:60,speed:65},   // no vale 1
        {x:2440,y:370,w:120,h:22,rangeX:0,rangeY:55,speed:70},   // no vale 4
      ],
      trampolines:[{x:1820,y:510},{x:2300,y:510}],  // nos vales mais fundos
      secrets:[{x:3020,y:248,kind:"estrela",points:35}],
    },
    {
      name: "Nível 16 — O Direito à Privacidade",
      theme:15, quizTheme:"privacidade", worldW:3850,
      spawn:{x:120,y:464}, doorX:3650,
      // ══ LABIRINTO VERTICAL — sobe e desce duas vezes ══
      // O percurso sobe alto, desce ao fundo, volta a subir ainda mais,
      // e finalmente desce para a porta. Plataformas estreitas.
      // Os jumpers estão nos topos — mais perigoso quanto mais alto.
      platforms:[
        {x:120,y:520,w:380,h:28},

        // 1ª subida
        {x:680,y:430,w:140,h:22},
        {x:860,y:340,w:130,h:22},
        {x:1040,y:250,w:130,h:22},
        {x:1220,y:170,w:140,h:22},          // 1º pico

        // 1ª descida
        {x:1420,y:260,w:130,h:22},
        {x:1600,y:370,w:130,h:22},
        {x:1780,y:460,w:130,h:22},          // 1º vale

        // 2ª subida — ainda mais alta
        {x:1980,y:360,w:130,h:22},
        {x:2170,y:260,w:130,h:22},
        {x:2360,y:170,w:130,h:22},
        {x:2550,y:100,w:140,h:22},          // 2º pico — topo absoluto

        // 2ª descida até à porta
        {x:2750,y:200,w:130,h:22},
        {x:2940,y:320,w:130,h:22},
        {x:3130,y:430,w:140,h:22},
        {x:3700,y:520,w:600,h:28},
      ],
      items:[
        {x:680,y:380,kind:"balao"},
        {x:1040,y:200,kind:"estrela"},
        {x:1220,y:120,kind:"medalha"},       // 1º pico
        {x:1780,y:410,kind:"duplosalto"},    // no vale — recompensa o risco
        {x:2550,y:50,kind:"brinquedo"},      // topo absoluto
        {x:120,y:470,kind:"heart"},
      ],
      malwares:[
        {x:860,y:312,vx:0,pattern:"mini"},
        {x:1220,y:142,vx:0,pattern:"jumper"},    // guarda 1º pico
        {x:1600,y:342,vx:100,pattern:"patrol"},
        {x:2170,y:232,vx:0,pattern:"patrol"},
        {x:2550,y:72,vx:0,pattern:"jumper"},     // guarda o topo absoluto
        {x:2940,y:292,vx:110,pattern:"patrol"},
      ],
      movingPlatforms:[
        {x:1300,y:215,w:110,h:22,rangeX:0,rangeY:70,speed:65},
        {x:2460,y:135,w:110,h:22,rangeX:0,rangeY:65,speed:75},
      ],
      trampolines:[{x:1880,y:510}],
      secrets:[{x:3130,y:378,kind:"estrela",points:35}],
    },
    {
      name: "Nível 17 — O Direito à Cultura",
      theme:16, quizTheme:"cultura", worldW:3900,
      spawn:{x:120,y:464}, doorX:3700,
      // ══ CONSTELAÇÕES — 4 grupos com personalidade diferente ══
      // Cada "constelação" (grupo de 3 plataformas) tem uma forma diferente:
      // A=triângulo, B=linha horizontal, C=L invertido, D=torre.
      platforms:[
        {x:120,y:520,w:380,h:28},

        // Constelação A — Triângulo
        {x:660,y:430,w:150,h:22},           // base esq
        {x:860,y:430,w:150,h:22},           // base dir
        {x:760,y:310,w:140,h:22},           // topo

        // Constelação B — Linha horizontal escalonada
        {x:1200,y:380,w:130,h:22},
        {x:1380,y:310,w:130,h:22},
        {x:1560,y:240,w:130,h:22},          // mais alta

        // Constelação C — L invertido
        {x:1900,y:420,w:200,h:22},          // braço horizontal longo
        {x:1900,y:300,w:130,h:22},          // braço vertical (esq)
        {x:2060,y:200,w:130,h:22},          // topo do L

        // Constelação D — Torre
        {x:2500,y:440,w:130,h:22},
        {x:2500,y:330,w:130,h:22},
        {x:2500,y:220,w:130,h:22},
        {x:2500,y:120,w:130,h:22},          // topo da torre

        {x:3120,y:360,w:140,h:22},
        {x:3750,y:520,w:600,h:28},
      ],
      items:[
        {x:760,y:260,kind:"balao"},          // topo constelação A
        {x:1560,y:190,kind:"estrela"},       // fim constelação B
        {x:2060,y:150,kind:"medalha"},       // topo constelação C
        {x:2500,y:70,kind:"duplosalto"},     // topo da torre D
        {x:3120,y:310,kind:"brinquedo"},
        {x:120,y:470,kind:"heart"},
      ],
      malwares:[
        {x:760,y:282,vx:0,pattern:"mini"},
        {x:1380,y:282,vx:0,pattern:"patrol"},
        {x:1900,y:272,vx:0,pattern:"patrol"},
        {x:2060,y:172,vx:0,pattern:"jumper"},
        {x:2500,y:192,vx:0,pattern:"jumper"},    // patrol na torre
        {x:2500,y:92,vx:0,pattern:"jumper"},     // jumper no topo
      ],
      movingPlatforms:[
        {x:1040,y:375,w:120,h:22,rangeX:100,rangeY:0,speed:80},   // ponte A→B
        {x:2280,y:360,w:120,h:22,rangeX:0,rangeY:80,speed:70},    // ponte C→D
        {x:2820,y:290,w:120,h:22,rangeX:110,rangeY:0,speed:95},   // depois da torre
      ],
      secrets:[{x:3120,y:308,kind:"estrela",points:35}],
    },
    {
      name: "Nível 18 — O Direito à Inclusão",
      theme:17, quizTheme:"deficiencia", worldW:3950,
      spawn:{x:120,y:464}, doorX:3700,
      // ══ ESTEIRA — TODAS AS PLATAFORMAS MÓVEIS ══ (mantido — mecânica única)
      // Adicionado: vilões nas plataformas móveis correctamente
      platforms:[
        {x:120,y:520,w:480,h:28},
        {x:3750,y:520,w:600,h:28},
      ],
      movingPlatforms:[
        {x:820,y:420,w:160,h:22,rangeX:140,rangeY:0,speed:55},
        {x:1060,y:330,w:150,h:22,rangeX:0,rangeY:110,speed:50},
        {x:1310,y:370,w:160,h:22,rangeX:120,rangeY:0,speed:70},
        {x:1560,y:260,w:150,h:22,rangeX:0,rangeY:130,speed:60},
        {x:1810,y:390,w:160,h:22,rangeX:110,rangeY:0,speed:80},
        {x:2060,y:300,w:145,h:22,rangeX:0,rangeY:120,speed:65},
        {x:2300,y:230,w:150,h:22,rangeX:160,rangeY:0,speed:90},
        {x:2550,y:360,w:145,h:22,rangeX:0,rangeY:100,speed:75},
        {x:2800,y:420,w:150,h:22,rangeX:130,rangeY:0,speed:100},
        {x:3040,y:300,w:145,h:22,rangeX:0,rangeY:120,speed:85},
        {x:3280,y:400,w:150,h:22,rangeX:140,rangeY:0,speed:95},
      ],
      items:[
        {x:820,y:370,kind:"balao"},
        {x:1310,y:220,kind:"estrela"},
        {x:1560,y:160,kind:"duplosalto"},
        {x:2300,y:130,kind:"medalha"},
        {x:3040,y:200,kind:"brinquedo"},
        {x:120,y:470,kind:"heart"},
      ],
      malwares:[
        // Colocados nas plataformas estáticas de arranque e chegada
        // e em plataformas móveis suficientemente largas
        {x:300,y:492,vx:150,pattern:"patrol"},
        {x:600,y:492,vx:-150,pattern:"patrol"},
        {x:3550,y:492,vx:140,pattern:"jumper"},
        {x:3700,y:492,vx:-140,pattern:"patrol"},
      ],
      trampolines:[{x:2460,y:510}],
      secrets:[{x:1810,y:250,kind:"estrela",points:30}],
    },
    {
      name: "Nível 19 — O Direito ao Ambiente",
      theme:18, quizTheme:"ambiente", worldW:4000,
      spawn:{x:120,y:464}, doorX:3800,
      // ══ FLORESTA — galhos em padrão orgânico, vãos variados ══
      // Muitas plataformas pequenas a diferentes alturas como ramos de árvore.
      // Alguns galhos muito altos exigem duplosalto ou trampolim.
      // Plataformas móveis simulam ramos a balançar no vento.
      platforms:[
        {x:120,y:520,w:380,h:28},

        // Zona 1 — entrada da floresta
        {x:680,y:430,w:130,h:22},
        {x:850,y:340,w:120,h:22},
        {x:1010,y:430,w:120,h:22},
        {x:1180,y:340,w:120,h:22},
        {x:1350,y:250,w:130,h:22},          // galho alto

        // Zona 2 — interior da floresta
        {x:1620,y:340,w:120,h:22},
        {x:1800,y:240,w:120,h:22},          // galho alto
        {x:1980,y:340,w:120,h:22},
        {x:2160,y:440,w:120,h:22},
        {x:2340,y:330,w:130,h:22},

        // Zona 3 — floresta densa
        {x:2620,y:240,w:120,h:22},          // galho mais alto
        {x:2800,y:340,w:120,h:22},
        {x:2980,y:430,w:120,h:22},
        {x:3160,y:320,w:130,h:22},
        {x:3360,y:230,w:130,h:22},          // galho final

        {x:3850,y:520,w:600,h:28},
      ],
      items:[
        {x:850,y:290,kind:"balao"},
        {x:1350,y:200,kind:"estrela"},
        {x:1800,y:190,kind:"medalha"},
        {x:2620,y:190,kind:"duplosalto"},
        {x:3360,y:180,kind:"brinquedo"},
        {x:120,y:470,kind:"heart"},
      ],
      malwares:[
        {x:680,y:402,vx:0,pattern:"mini"},
        {x:1010,y:402,vx:0,pattern:"mini"},
        {x:1350,y:222,vx:0,pattern:"patrol"},
        {x:1800,y:212,vx:0,pattern:"jumper"},
        {x:2340,y:302,vx:100,pattern:"patrol"},
        {x:2620,y:212,vx:0,pattern:"jumper"},
        {x:3160,y:292,vx:0,pattern:"patrol"},
        {x:3360,y:202,vx:0,pattern:"jumper"},
      ],
      movingPlatforms:[
        {x:1500,y:300,w:110,h:22,rangeX:80,rangeY:0,speed:70},   // ramo a balançar 1
        {x:2470,y:290,w:110,h:22,rangeX:0,rangeY:60,speed:60},   // ramo a subir/descer
        {x:3540,y:280,w:110,h:22,rangeX:90,rangeY:0,speed:80},   // ramo final
      ],
      trampolines:[{x:2160,y:510},{x:3640,y:510}],
      secrets:[{x:2980,y:378,kind:"estrela",points:35},{x:1980,y:288,kind:"heart"}],
    },
    {
      name: "Nível 20 — Os Direitos Digitais",
      theme:19, quizTheme:"digital", worldW:4100,
      spawn:{x:120,y:464}, doorX:3900,
      // ══ CIRCUITO DIGITAL — blocos longos com pares de vilões ══
      // Plataformas longas horizontais (tipo circuito impresso).
      // Em cada bloco há 2 vilões a patrulhar em sentidos opostos.
      // O jogador tem de escolher o momento certo para atravessar cada bloco.
      // O nível mais longo e mais difícil — todos os tipos de vilão.
      platforms:[
        {x:120,y:520,w:380,h:28},

        // Bloco 1 — longo, dois patrol
        {x:860,y:420,w:320,h:22},

        // Bloco 2 — alto, dois patrol
        {x:1500,y:270,w:300,h:22},

        // Bloco 3 — médio, patrol + jumper
        {x:2080,y:360,w:260,h:22},

        // Bloco 4 — alto, dois patrol
        {x:2660,y:240,w:300,h:22},

        // Bloco 5 — longo, patrol + jumper
        {x:3280,y:340,w:280,h:22},

        // Final
        {x:3950,y:520,w:600,h:28},
      ],
      items:[
        {x:800,y:370,kind:"balao"},
        {x:1400,y:310,kind:"estrela"},
        {x:1500,y:220,kind:"duplosalto"},
        {x:2080,y:310,kind:"medalha"},
        {x:2660,y:190,kind:"brinquedo"},
        {x:120,y:470,kind:"heart"},
      ],
      malwares:[
        // Bloco 1 — dois patrol em sentidos opostos
        {x:760,y:392,vx:160,pattern:"patrol"},
        {x:960,y:392,vx:-160,pattern:"patrol"},
        // Bloco 2 — dois patrol
        {x:1440,y:242,vx:150,pattern:"patrol"},
        {x:1560,y:242,vx:-150,pattern:"patrol"},
        // Bloco 3 — patrol + jumper
        {x:2000,y:332,vx:130,pattern:"patrol"},
        {x:2160,y:332,vx:0,pattern:"jumper"},
        // Bloco 4 — dois patrol rápidos
        {x:2590,y:212,vx:145,pattern:"patrol"},
        {x:2730,y:212,vx:-145,pattern:"patrol"},
        // Bloco 5 — patrol + jumper final
        {x:3200,y:312,vx:140,pattern:"patrol"},
        {x:3360,y:312,vx:0,pattern:"jumper"},
      ],
      movingPlatforms:[
        {x:1290,y:345,w:120,h:22,rangeX:0,rangeY:75,speed:80},   // elevador 1→2
        {x:1870,y:315,w:120,h:22,rangeX:100,rangeY:0,speed:90},  // ponte 2→3
        {x:2460,y:300,w:120,h:22,rangeX:0,rangeY:70,speed:85},   // elevador 3→4
        {x:3060,y:290,w:120,h:22,rangeX:110,rangeY:0,speed:100}, // ponte 4→5
      ],
      trampolines:[{x:1750,y:510},{x:3780,y:510}],
      secrets:[
        {x:2080,y:308,kind:"estrela",points:30},
        {x:3280,y:288,kind:"medalha"},
        {x:3650,y:248,kind:"estrela",points:40},
      ],
    },
  ];
  ...
  let levelDecorGfx;         // decorações específicas por nível (roda gigante, etc.)
  ...
  function spawnShields(scene, L) {
    ...
    const plats = L.platforms.filter(p => p.w < 600 && Math.abs(p.x - spawnX) > 200);
    if (!plats.length) return;

    const sorted = [...plats].sort((a, b) => a.x - b.x);

    // Excluir plataformas que já têm item de L.items OU secret nas proximidades (margem 60px)
    const secrets = L.secrets || [];
    const p = sorted.find(pl => {
      const left  = pl.x - pl.w / 2 - 60;
      const right = pl.x + pl.w / 2 + 60;
      const hasItem   = L.items.some(it => it.x >= left && it.x <= right);
      const hasSecret = secrets.some(s  => s.x  >= left && s.x  <= right);
      return !hasItem && !hasSecret;
    });
    if (!p) return;
  ...
  const PLAT_COLORS=[
    [0x0d3878,0x1e5cb8,0x6aa8ff], // tema0  · N1  azul clássico
    [0x6a1800,0xc04018,0xff8850], // tema1  · N2  crepúsculo laranja
    [0x004858,0x0090a0,0x60e8f0], // tema2  · N3  aqua profundo
    [0x780840,0xc03070,0xffa0c8], // tema3  · N4  rosa
    [0x200060,0x4810a0,0xc080ff], // tema4  · N5  lilás noturno
    [0x003a3a,0x008888,0x40e8e0], // tema5  · N6  turquesa
    [0x6a1800,0xb84010,0xff9050], // tema6  · N7  laranja quente
    [0x001040,0x0848a0,0x60b8f8], // tema7  · N8  azul noturno
    [0x580020,0xa01060,0xff80b8], // tema8  · N9  magenta rico
    [0x081808,0x185c28,0x70d870], // tema9  · N10 floresta verde
    [0x5a0800,0xaa2000,0xff5030], // tema10 · N11 vermelho vulcão
    [0x001828,0x004878,0x20a0d0], // tema11 · N12 oceano profundo
    [0x281800,0x704010,0xc09040], // tema12 · N13 dourado árido
    [0x080828,0x1818a0,0x6060e0], // tema13 · N14 azul meia-noite
    [0x004010,0x108040,0x50e060], // tema14 · N15 verde primavera
    [0x280040,0x6000a0,0xc040ff], // tema15 · N16 violeta escuro
    [0x502000,0xa05000,0xffb030], // tema16 · N17 âmbar outonal
    [0x002030,0x006860,0x20d0b0], // tema17 · N18 verde-azulado noturno
    [0x280030,0x800060,0xff40b0], // tema18 · N19 púrpura aurora
    [0x7a4a00,0xd9921a,0xffd86a], // tema19 · N20 FINAL dourado festivo
  ];
  ...
  function initBackground(scene){
    ...
    levelDecorGfx =scene.add.graphics().setDepth(-48).setScrollFactor(0.18); // entre hills e far
  ...
  function drawFarLayer(themeIdx, worldW){
    ...
    const BUILD_PALETTES = [
      { walls:[0x1a3060,0x0a2050,0x162848,0x0e1e3a], wins:[0xffd700,0x80d0ff,0xffe880,0xff8040] }, // tema0  · N1  azul
      { walls:[0x5a2800,0x3a1800,0x6a3010,0x2e1200], wins:[0xffd700,0xffe880,0xff8040,0xffc060] }, // tema1  · N2  crepúsculo
      { walls:[0x005040,0x003830,0x006858,0x002e28], wins:[0x80ffe0,0x40d4b8,0xffd700,0xb0fff0] }, // tema2  · N3  aqua
      { walls:[0x5a1030,0x3e0820,0x6e1840,0x2e0818], wins:[0xff80c0,0xffd700,0xffb0d0,0xff6090] }, // tema3  · N4  rosa
      { walls:[0x1a0840,0x0a1a40,0x200830,0x0a2040], wins:[0xffd700,0xffe880,0x80d0ff,0xff8040] }, // tema4  · N5  lilás
      { walls:[0x103820,0x082810,0x185030,0x062010], wins:[0xa0ffb0,0x40c060,0xffd700,0x80ff90] }, // tema5  · N6  turquesa
      { walls:[0x5a1800,0x401000,0x682000,0x300c00], wins:[0xffa060,0xffd700,0xff8040,0xffb880] }, // tema6  · N7  laranja
      { walls:[0x083060,0x042048,0x0c3870,0x021838], wins:[0x80d0ff,0x2898e0,0xffd700,0xb0e8ff] }, // tema7  · N8  azul noturno
      { walls:[0x500828,0x380518,0x601030,0x280410], wins:[0xffa0c8,0xffd700,0xff80c0,0xffc8de] }, // tema8  · N9  magenta
      { walls:[0x104020,0x082e10,0x185028,0x061c08], wins:[0xb0ff80,0x30a050,0xffd700,0xd0ffb0] }, // tema9  · N10 floresta
      { walls:[0x4a0c08,0x320800,0x5c1000,0x200400], wins:[0xff6040,0xffd700,0xff8060,0xffb0a0] }, // tema10 · N11 vermelho vulcão
      { walls:[0x001828,0x000e18,0x002038,0x00101e], wins:[0x40c0e0,0x80e8ff,0xffd700,0xa0f0ff] }, // tema11 · N12 oceano
      { walls:[0x281800,0x180c00,0x382000,0x100800], wins:[0xd4a040,0xffe880,0xffd700,0xc09030] }, // tema12 · N13 árido
      { walls:[0x080828,0x040418,0x0c0c38,0x020210], wins:[0x6060e0,0xa0a0ff,0xffd700,0x8080ff] }, // tema13 · N14 meia-noite
      { walls:[0x004010,0x002808,0x006018,0x001808], wins:[0x50e060,0xa0ff80,0xffd700,0x80ff60] }, // tema14 · N15 primavera
      { walls:[0x280040,0x180028,0x380058,0x100018], wins:[0xc040ff,0xe080ff,0xffd700,0xd060ff] }, // tema15 · N16 violeta
      { walls:[0x502000,0x381400,0x603000,0x280c00], wins:[0xffb030,0xffd700,0xffc840,0xff9020] }, // tema16 · N17 âmbar
      { walls:[0x002030,0x001020,0x003040,0x000c18], wins:[0x20d0b0,0x40ffe0,0xffd700,0x60ffcc] }, // tema17 · N18 verde-azulado
      { walls:[0x280030,0x180020,0x380040,0x100018], wins:[0xff40b0,0xff80d0,0xffd700,0xff60c0] }, // tema18 · N19 púrpura
      { walls:[0x7a3a00,0x5a2a00,0x8a4a10,0x3e1c00], wins:[0xffd700,0xfff0a0,0xff9040,0xffe880] }, // tema19 · N20 FINAL dourado
    ];
  ...
  function applyBackground(scene,themeIdx,worldW,hazardDefs=[]){
    ...
    // ── ÁRVORES e CASINHAS ─────────────────────────────────────────
    // Todos os níveis usam o mesmo conjunto de árvores e casinhas.
    if (true) {
      // Árvores
      ...
      treePositions.filter(tx=>tx<worldW-40).forEach((tx,ti)=>{
        ...
        const tcL = treeColors[(ti+1)%treeColors.length];
        const base= 510;
        const th  = 60+((ti*41)%30);
        const tw  = 24+((ti*17)%10);
      ...
      houseX.filter(hx=>hx<worldW-80).forEach((hx,hi)=>{
        ...
        const hw  = 48, hh  = 36, base= 510;
        const hy  = base - hh, cx  = hx + hw/2;
        hillsGraphics.fillStyle(0x000000,0.10);
        hillsGraphics.fillEllipse(cx, base+2, hw+8, 6);
        const chimX = cx + 10;
        const chimTop = hy - 22, chimBot = hy - 4;
        ...
        [[hx+6, winY],[hx+hw-17, winY]].forEach(([wx,wy])=>{
          hillsGraphics.fillStyle(0xffffff,0.6); hillsGraphics.fillRect(wx-1,wy-1,13,12);
          hillsGraphics.fillStyle(0xc8eaff,0.9); hillsGraphics.fillRect(wx,wy,12,11);
          hillsGraphics.fillStyle(0xffffff,0.7); hillsGraphics.fillRect(wx,wy+4,12,2);
          hillsGraphics.fillRect(wx+5,wy,2,11);
          hillsGraphics.fillStyle(0xffffff,0.35); hillsGraphics.fillRect(wx+1,wy+1,4,4);
        });
        const doorW=12, doorH=18;
        const doorX=cx-doorW/2, doorY=base-doorH;
        hillsGraphics.fillStyle(0x5a3010,1); hillsGraphics.fillRect(doorX-1,doorY-1,doorW+2,doorH+1);
        hillsGraphics.fillStyle(0x8b5a2a,1); hillsGraphics.fillRect(doorX,doorY,doorW,doorH);
        hillsGraphics.fillStyle(0x8b5a2a,1); hillsGraphics.fillEllipse(cx,doorY,doorW,8);
        hillsGraphics.fillStyle(0x5a3010,0.4); hillsGraphics.fillEllipse(cx,doorY,doorW+2,8);
        hillsGraphics.fillStyle(0xffd700,1); hillsGraphics.fillCircle(doorX+doorW-3,doorY+doorH/2,2);
      });
    } // fim if themeIdx !== 0
  ...
  // ── Decoração específica por nível ────────────────────────────
  function drawLevelDecor(themeIdx, worldW) {
    if (!levelDecorGfx) return;
    levelDecorGfx.clear();
    if (themeIdx === 0) drawLevel1FestivalDecor(worldW);
  }

  // ── Nível 1 — Parque de diversões / Festa ─────────────────────
  // Substitui as casinhas e árvores genéricas por elementos de festa:
  // roda gigante, tendas coloridas, bandeiras, balões presos, foguetes de confetis
  function drawLevel1FestivalDecor(worldW) {
    const g = levelDecorGfx;
    const base = 510; // Y do chão

    // ── Roda Gigante — elemento icónico, lado esquerdo ────────────
    // Posição fixa no início do mundo (sempre visível no spawn)
    const wx = 340, wy = 340, wr = 90;

    // Sombra da roda no chão
    g.fillStyle(0x000000, 0.12);
    g.fillEllipse(wx, base, wr * 1.6, 18);

    // Suportes em A (dois pés)
    g.fillStyle(0x8b5a2a, 0.90);
    g.fillRect(wx - 10, wy + wr - 4, 8, base - wy - wr + 4);
    g.fillRect(wx + 2,  wy + wr - 4, 8, base - wy - wr + 4);
    // Travessão horizontal a meio dos suportes
    g.fillStyle(0x7a4a20, 0.85);
    g.fillRect(wx - 12, wy + wr + 40, 24, 7);

    // Aro exterior da roda — dois anéis concêntricos
    g.lineStyle(8, 0xe8a820, 0.92);
    g.strokeCircle(wx, wy, wr);
    g.lineStyle(4, 0xffd700, 0.65);
    g.strokeCircle(wx, wy, wr - 10);

    // Raios (8 raios dourados)
    g.lineStyle(3, 0xffd700, 0.70);
    for (let ri = 0; ri < 8; ri++) {
      const a = Math.PI * 2 * ri / 8;
      g.beginPath();
      g.moveTo(wx, wy);
      g.lineTo(wx + Math.cos(a) * (wr - 2), wy + Math.sin(a) * (wr - 2));
      g.strokePath();
    }

    // Centro da roda — cubo dourado com brilho
    g.fillStyle(0xffd700, 1);
    g.fillCircle(wx, wy, 12);
    g.fillStyle(0xffe880, 0.8);
    g.fillCircle(wx - 3, wy - 3, 5);

    // Gondolas (6 casinhas nas pontas dos raios)
    const gondolaColors = [0xff4466, 0x4488ff, 0xffdd00, 0x44cc44, 0xff8800, 0xcc44ff];
    for (let gi = 0; gi < 6; gi++) {
      const ga = Math.PI * 2 * gi / 6 - Math.PI * 0.15;
      const gx = wx + Math.cos(ga) * (wr - 4);
      const gy = wy + Math.sin(ga) * (wr - 4);
      const gc = gondolaColors[gi];
      // Caixinha da gondola
      g.fillStyle(gc, 0.92);
      g.fillRect(gx - 9, gy - 7, 18, 14);
      // Telhinho triangular
      g.fillStyle(Phaser.Display.Color.IntegerToColor(gc).darken(20).color, 0.95);
      g.fillTriangle(gx - 11, gy - 7, gx + 11, gy - 7, gx, gy - 16);
      // Janela
      g.fillStyle(0xc8f0ff, 0.85);
      g.fillRect(gx - 4, gy - 4, 8, 7);
      // Brilho da janela
      g.fillStyle(0xffffff, 0.50);
      g.fillRect(gx - 3, gy - 3, 3, 3);
    }

    // ── Tenda 1 — listrada vermelha/branca, estrutura de circo ────
    const t1x = 700, t1y = base;
    // Poste central
    g.fillStyle(0x8b3a00, 0.90);
    g.fillRect(t1x - 3, t1y - 110, 6, 110);
    // Bandeirinha no topo
    g.fillStyle(0xff2244, 0.95);
    g.fillTriangle(t1x, t1y - 118, t1x, t1y - 106, t1x + 14, t1y - 112);
    // Teto da tenda — triângulos alternados vermelho/branco
    const tendaW = 90;
    for (let ti = 0; ti < 6; ti++) {
      const tx1 = t1x - tendaW + ti * (tendaW * 2 / 6);
      const tx2 = t1x - tendaW + (ti + 1) * (tendaW * 2 / 6);
      g.fillStyle(ti % 2 === 0 ? 0xff2244 : 0xffffff, 0.92);
      g.fillTriangle(tx1, t1y - 60, tx2, t1y - 60, (tx1 + tx2) / 2, t1y - 90);
    }
    // Base da tenda (parede)
    g.fillStyle(0xffe0e8, 0.85);
    g.fillRect(t1x - tendaW, t1y - 60, tendaW * 2, 60);
    // Listras verticais na parede
    for (let li = 0; li < 6; li++) {
      g.fillStyle(0xff9090, 0.30);
      g.fillRect(t1x - tendaW + li * (tendaW * 2 / 6), t1y - 60, (tendaW * 2 / 6) * 0.4, 60);
    }
    // Entrada arqueada
    g.fillStyle(0xc80030, 0.95);
    g.fillRect(t1x - 18, t1y - 40, 36, 40);
    g.fillStyle(0xc80030, 0.95);
    g.fillEllipse(t1x, t1y - 40, 36, 22);
    // Faixa "FESTA" em cima da entrada
    g.fillStyle(0xffd700, 0.90);
    g.fillRect(t1x - 26, t1y - 68, 52, 10);

    // ── Tenda 2 — azul/amarela, mais afastada ─────────────────────
    const t2x = 1580, t2y = base;
    g.fillStyle(0x5a3000, 0.90);
    g.fillRect(t2x - 3, t2y - 100, 6, 100);
    g.fillStyle(0x1144ff, 0.95);
    g.fillTriangle(t2x, t2y - 108, t2x, t2y - 96, t2x + 13, t2y - 102);
    const t2W = 80;
    for (let ti = 0; ti < 5; ti++) {
      const tx1 = t2x - t2W + ti * (t2W * 2 / 5);
      const tx2 = t2x - t2W + (ti + 1) * (t2W * 2 / 5);
      g.fillStyle(ti % 2 === 0 ? 0x2266ff : 0xffe840, 0.92);
      g.fillTriangle(tx1, t2y - 52, tx2, t2y - 52, (tx1 + tx2) / 2, t2y - 80);
    }
    g.fillStyle(0xe8f0ff, 0.85);
    g.fillRect(t2x - t2W, t2y - 52, t2W * 2, 52);
    g.fillStyle(0x2244cc, 0.95);
    g.fillRect(t2x - 16, t2y - 36, 32, 36);
    g.fillStyle(0x2244cc, 0.95);
    g.fillEllipse(t2x, t2y - 36, 32, 20);

    // ── Tenda 3 — verde/laranja, lado direito do nível ────────────
    const t3x = 1980, t3y = base;
    g.fillStyle(0x5a3000, 0.90);
    g.fillRect(t3x - 3, t3y - 105, 6, 105);
    g.fillStyle(0x22aa44, 0.95);
    g.fillTriangle(t3x, t3y - 113, t3x, t3y - 101, t3x + 13, t3y - 107);
    const t3W = 85;
    for (let ti = 0; ti < 5; ti++) {
      const tx1 = t3x - t3W + ti * (t3W * 2 / 5);
      const tx2 = t3x - t3W + (ti + 1) * (t3W * 2 / 5);
      g.fillStyle(ti % 2 === 0 ? 0x22aa44 : 0xff8800, 0.92);
      g.fillTriangle(tx1, t3y - 56, tx2, t3y - 56, (tx1 + tx2) / 2, t3y - 84);
    }
    g.fillStyle(0xe8ffe8, 0.85);
    g.fillRect(t3x - t3W, t3y - 56, t3W * 2, 56);
    g.fillStyle(0x118833, 0.95);
    g.fillRect(t3x - 17, t3y - 38, 34, 38);
    g.fillStyle(0x118833, 0.95);
    g.fillEllipse(t3x, t3y - 38, 34, 21);

    // ── Bandeiras festivas em cordas (3 cordas ao longo do nível) ─
    const flagColors = [0xff2244, 0xffe840, 0x2266ff, 0x22cc44, 0xff8800, 0xcc44ff, 0xffffff];
    // Corda 1 — baixa, perto do chão
    const rope1Points = [[100, base-90], [400, base-100], [750, base-85], [1100, base-95], [1450, base-80]];
    for (let ri = 0; ri < rope1Points.length - 1; ri++) {
      const [x1, y1] = rope1Points[ri], [x2, y2] = rope1Points[ri + 1];
      // Linha da corda
      g.lineStyle(1.5, 0x6a4a20, 0.55);
      g.beginPath(); g.moveTo(x1, y1); g.lineTo(x2, y2); g.strokePath();
      // Bandeirinhas triangulares ao longo da corda
      const segLen = Math.hypot(x2 - x1, y2 - y1);
      const numFlags = Math.floor(segLen / 48);
      for (let fi = 0; fi < numFlags; fi++) {
        const t = (fi + 0.5) / numFlags;
        const fx = x1 + (x2 - x1) * t;
        const fy = y1 + (y2 - y1) * t;
        const fc2 = flagColors[(ri * 4 + fi) % flagColors.length];
        g.fillStyle(fc2, 0.88);
        g.fillTriangle(fx - 8, fy, fx + 8, fy, fx, fy + 16);
        // Borda escura da bandeirinha
        g.lineStyle(1, 0x00000040, 0.30);
        g.strokeTriangle(fx - 8, fy, fx + 8, fy, fx, fy + 16);
      }
    }
    // Corda 2 — mais alta, segundo terço do nível
    const rope2Points = [[1100, base-120], [1400, base-130], [1700, base-118], [2000, base-125], [2300, base-115]];
    for (let ri = 0; ri < rope2Points.length - 1; ri++) {
      const [x1, y1] = rope2Points[ri], [x2, y2] = rope2Points[ri + 1];
      g.lineStyle(1.5, 0x6a4a20, 0.50);
      g.beginPath(); g.moveTo(x1, y1); g.lineTo(x2, y2); g.strokePath();
      const segLen = Math.hypot(x2 - x1, y2 - y1);
      const numFlags = Math.floor(segLen / 44);
      for (let fi = 0; fi < numFlags; fi++) {
        const t = (fi + 0.5) / numFlags;
        const fx = x1 + (x2 - x1) * t;
        const fy = y1 + (y2 - y1) * t;
        const fc2 = flagColors[(ri * 3 + fi + 2) % flagColors.length];
        g.fillStyle(fc2, 0.85);
        g.fillTriangle(fx - 7, fy, fx + 7, fy, fx, fy + 14);
      }
    }

    // ── Balões presos no chão (estacas) ───────────────────────────
    const balloonStakes = [
      { x: 480,  color: 0xff4488 },
      { x: 820,  color: 0xffe840 },
      { x: 1240, color: 0x44aaff },
      { x: 1640, color: 0x44ee66 },
      { x: 2050, color: 0xff8800 },
    ];
    balloonStakes.forEach(({ x: bsx, color: bsc }) => {
      // Fio fino
      g.lineStyle(1.2, 0x888888, 0.70);
      g.beginPath(); g.moveTo(bsx, base); g.lineTo(bsx + 4, base - 68); g.strokePath();
      // Corpo do balão
      const bGrad = g; // usar fillStyle direto
      g.fillStyle(bsc, 0.90);
      g.fillEllipse(bsx + 4, base - 80, 26, 32);
      // Brilho no balão
      g.fillStyle(0xffffff, 0.38);
      g.fillEllipse(bsx - 2, base - 88, 10, 14);
      // Nozinho base
      g.fillStyle(Phaser.Display.Color.IntegerToColor(bsc).darken(25).color, 0.90);
      g.fillCircle(bsx + 4, base - 64, 4);
    });

    // ── Confetis no chão (substituem as flores genéricas) ─────────
    // São desenhados no decorGraphics (já existe), mas adicionamos
    // uns extras festivos no levelDecorGfx também — mais densos e coloridos
    const confColors = [0xff2244, 0xffe840, 0x2266ff, 0x44cc44, 0xff8800, 0xcc44ff, 0xffffff, 0xff80c0];
    for (let ci = 0; ci < Math.floor(worldW / 18); ci++) {
      const cx2 = 12 + ci * 18 + (ci % 5) * 4;
      const cy2 = base - 4 - (ci % 3) * 3;
      const cc2 = confColors[ci % confColors.length];
      g.fillStyle(cc2, 0.70);
      // Alternar entre retângulo (confetis clássicos) e círculo
      if (ci % 3 === 0) {
        g.fillRect(cx2 - 4, cy2 - 3, 8, 5);
      } else if (ci % 3 === 1) {
        g.fillCircle(cx2, cy2, 4);
      } else {
        g.fillTriangle(cx2 - 4, cy2 + 2, cx2 + 4, cy2 + 2, cx2, cy2 - 5);
      }
    }

    // ── Serpentinas enroladas no chão (espirais curvas coloridas) ─
    const serpentineColors = [0xff4488, 0x44aaff, 0xffe840, 0x44ee66];
    for (let si = 0; si < 8; si++) {
      const sx = 200 + si * Math.floor(worldW / 9);
      const sc2 = serpentineColors[si % serpentineColors.length];
      g.lineStyle(2.5, sc2, 0.65);
      g.beginPath();
      for (let t = 0; t <= Math.PI * 2.5; t += 0.15) {
        const px2 = sx + Math.cos(t) * (12 + t * 3);
        const py2 = (base - 8) + Math.sin(t) * 5;
        t === 0 ? g.moveTo(px2, py2) : g.lineTo(px2, py2);
      }
      g.strokePath();
    }
  }
